...
ifeq ($(SIGN),NONE)
  ...
  # When CERT_CHAIN_GEN is enabled, use the generated leaf key for signing
  ifneq ($(CERT_CHAIN_GEN),)
    PRIVATE_KEY=test-dummy-ca/leaf-prvkey.der
    # Also need the original key for initial generation
    WOLFBOOT_PRIVATE_KEY=wolfboot_signing_private_key.der
  else
    PRIVATE_KEY=wolfboot_signing_private_key.der
    WOLFBOOT_PRIVATE_KEY=$(PRIVATE_KEY)
  endif
...
$(WOLFBOOT_PRIVATE_KEY):
	$(Q)$(MAKE) keytools_check
	$(Q)(test $(SIGN) = NONE) || ($(SIGN_ENV) "$(KEYGEN_TOOL)" $(KEYGEN_OPTIONS) -g $(WOLFBOOT_PRIVATE_KEY)) || true
	$(Q)(test $(SIGN) = NONE) && (echo "// SIGN=NONE" >  src/keystore.c) || true
	$(Q)(test "$(FLASH_OTP_KEYSTORE)" = "1") && (make -C tools/keytools/otp) || true
	$(Q)(test $(SIGN) = NONE) || (test "$(CERT_CHAIN_VERIFY)" = "") || (test "$(CERT_CHAIN_GEN)" = "") || (tools/scripts/sim-gen-dummy-chain.sh --algo $(CERT_CHAIN_GEN_ALGO) --leaf $(WOLFBOOT_PRIVATE_KEY))

# When using CERT_CHAIN_GEN, the actual signing key is generated by the cert chain script
$(PRIVATE_KEY): $(WOLFBOOT_PRIVATE_KEY)
